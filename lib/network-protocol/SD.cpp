...
fujiError_t NetworkProtocolSD::open_file_handle()
{
    ...
    Debug_printf("NetworkProtocolSD::open_file_handle(file: \"%s\" mode: \"%s\") error: %d\r\n",
                 opened_url->path.c_str(), mode, (int) last_error());
...
void NetworkProtocolSD::errno_to_error()
{
    ...
    case ENOBUFS:
    case ENOMEM:
        set_error(NDEV_STATUS::COULD_NOT_ALLOCATE_BUFFERS);
        break;
    default:
...
fujiError_t NetworkProtocolSD::read_file_handle(uint8_t *buf, unsigned short len)
{
    ...
    return NDEV_STATUS::SUCCESS != last_error() ? FUJI_ERROR::UNSPECIFIED : FUJI_ERROR::NONE;
}
