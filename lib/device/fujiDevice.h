...
protected:
    ...
    virtual fujiError_t transaction_get(void *data, size_t len) = 0;
    virtual void transaction_put(const void *data, size_t len, fujiError_t err=FUJI_ERROR::NONE) = 0;
    ...
    fujiError_t validate_host_slot(uint8_t slot, const char *dmsg=nullptr);
    fujiError_t validate_device_slot(uint8_t slot, const char *dmsg = nullptr);
...
public:
    ...
    virtual fujiError_t fujicmd_mount_all_success();
    virtual void fujicmd_reset();
    fujiError_t fujicmd_mount_host_success(unsigned hostSlot);
    virtual void fujicmd_net_scan_networks();
    void fujicmd_net_scan_result(uint8_t index);
    void fujicmd_net_get_ssid();
    fujiError_t fujicmd_net_set_ssid_success(const char *ssid, const char *password, bool save);
    void fujicmd_net_get_wifi_enabled();
    virtual fujiError_t fujicmd_mount_disk_image_success(uint8_t deviceSlot, disk_access_flags_t access_mode);
    fujiError_t fujicmd_unmount_disk_image_success(uint8_t deviceSlot);
    void fujicmd_image_rotate();
    fujiError_t fujicmd_open_directory_success(uint8_t hostSlot);
    ...
    fujiError_t fujicmd_copy_file_success(uint8_t sourceSlot, uint8_t destSlot, std::string copySpec);
    ...
    virtual fujiError_t fujicmd_set_device_filename_success(uint8_t deviceSlot, uint8_t host,
    ...
    fujiError_t fujicmd_unmount_host_success(uint8_t hostSlot);
    ...
    fujiError_t fujicore_open_app_key(uint16_t creator, uint8_t app, uint8_t key,
                                      appkey_mode mode, uint8_t reserved);
    SSIDInfo fujicore_net_scan_result(uint8_t index, fujiError_t *err=nullptr);
    ...
    fujiError_t fujicore_open_directory_success(uint8_t hostSlot, const std::string &dirpath);
    fujiError_t fujicore_open_directory_success(uint8_t hostSlot, const std::string &dirpath,
    ...
    fujiError_t fujicore_set_device_filename_success(uint8_t deviceSlot, uint8_t host,
                                                     disk_access_flags_t mode, std::string filename);
    std::optional<std::string> fujicore_get_device_filename(uint8_t slot);
    virtual fujiError_t fujicore_mount_disk_image_success(uint8_t deviceSlot,
                                                          disk_access_flags_t access_mode);
    fujiError_t fujicore_unmount_disk_image_success(uint8_t deviceSlot);
    fujiError_t fujicore_mount_host_success(unsigned hostSlot);
    fujiError_t fujicore_mount_all_success();
    void fujicore_net_scan_networks();
    fujiError_t fujicore_net_set_ssid_success(const char *ssid, const char *password, bool save);
