...
fujiError_t MediaTypePO::read(uint32_t blockNum, uint16_t *count, uint8_t* buffer)
{
...
if (blockNum == 0 || blockNum != last_block_num + 1) // example optimization, only do seek if not reading next block -tschak
  ...
  return readsize != *count ? FUJI_ERROR::UNSPECIFIED : FUJI_ERROR::NONE;
}

fujiError_t MediaTypePO::write(uint32_t blockNum, uint16_t *count, uint8_t* buffer)
{
...
fujiError_t MediaTypePO::write_sector(int track, int sector, uint8_t *buffer)
{
  Debug_printf("\r\nProDOS disk needs to write sector!");
  return FUJI_ERROR::UNSPECIFIED;
}

fujiError_t MediaTypePO::format(uint16_t *responsesize)
{
    return FUJI_ERROR::UNSPECIFIED;
}
