...
private:
    ...
    fujiError_t _load_atx_data(atx_header_t &atx_hdr);
    fujiError_t _load_atx_record();
    fujiError_t _load_atx_track_record(uint32_t length);
    int _load_atx_track_chunk(track_header_t &trk_hdr, AtxTrack &track);

    fujiError_t _load_atx_chunk_sector_list(chunk_header_t &chunk_hdr, AtxTrack &track);
    fujiError_t _load_atx_chunk_sector_data(chunk_header_t &chunk_hdr, AtxTrack &track);
    fujiError_t _load_atx_chunk_weak_sector(chunk_header_t &chunk_hdr, AtxTrack &track);
    fujiError_t _load_atx_chunk_extended_sector(chunk_header_t &chunk_hdr, AtxTrack &track);
    fujiError_t _load_atx_chunk_unknown(chunk_header_t &chunk_hdr, AtxTrack &track);

    fujiError_t _copy_track_sector_data(uint8_t tracknum, uint8_t sectornum, uint16_t sectorsize);
