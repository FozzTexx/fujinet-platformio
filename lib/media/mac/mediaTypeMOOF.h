...
public:
    ...
    bool read(uint32_t blockNum, uint8_t *buffer) override { return true; };
    bool write(uint32_t blockNum, uint8_t *buffer) override { return true; };

    bool format(uint16_t *responsesize) override { return false; };

    mediatype_t mount(FILE *f, uint32_t disksize) override;
    mediatype_t mount(FILE *f) { return mount(f, 0); };
    void unmount() override;

    bool status() override { return (_media_fileh != nullptr); }
