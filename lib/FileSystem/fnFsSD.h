...
public:
    ...
    fujiError_t start();
#else
    fujiError_t start(const char *sd_path = nullptr);
#endif
    ...
    fujiError_t mkdir(const char* path) override;
    fujiError_t rmdir(const char* path) override;
    ...
    fujiError_t create_path(const char *path);
