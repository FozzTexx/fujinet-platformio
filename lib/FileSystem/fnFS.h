...
public:
    ...
    virtual fujiError_t remove(const char* path) = 0;

    virtual fujiError_t rename(const char* pathFrom, const char* pathTo) = 0;

    virtual bool is_dir(const char *path) = 0;
    virtual fujiError_t mkdir(const char* path) = 0;
    virtual fujiError_t rmdir(const char* path) = 0;
    ...
    virtual fujiError_t dir_open(const char *path, const char *pattern, uint16_t diroptions) = 0;
    ...
    // Sets current position in directory stream. Returns FUJI_ERROR::UNSPECIFIED on error.
    virtual fujiError_t dir_seek(uint16_t position) = 0;
};
